...
const ModuleRow = ({ m, done }) => {
  const title = useContentText(`course.module.${m.id}.title`);
  const blurb = useContentText(`course.module.${m.id}.blurb`);
    ...
    >
      ...
      <div className="flex items-start gap-4 p-5 sm:p-6 border-b border-stroke/70">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-paper-dim text-sm leading-relaxed mt-1">{blurb}</p>
        </div>
        {!empty && (
  ...
  return (
    <div className="px-5 sm:px-8 lg:px-12 py-10 sm:py-14 max-w-4xl mx-auto">
      ...
      <div className="space-y-5">
        {modules.map((m) => <ModuleRow key={m.id} m={m} done={done} />)}
      </div>
